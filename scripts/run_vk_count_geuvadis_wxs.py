...
# wxs_df = wxs_df.drop_duplicates(subset=["Sample"])  # BAD
wxs_df = wxs_df[["Sample", "url"]].groupby("Sample").agg(list).reset_index()
...
def download_sequencing_total(
    ...
    sample_out_folder = os.path.join(sequencing_data_out_base, "wxs", sample)
    ...
    # Get all _1 and _2 FASTQ files
    files_1 = sorted(glob.glob(os.path.join(sample_out_folder, "*_1.fastq.gz")))
    files_2 = sorted(glob.glob(os.path.join(sample_out_folder, "*_2.fastq.gz")))

    # Ensure the base names match (e.g., SRR100124 in both _1 and _2 files)
    def base_name(file):
        return os.path.basename(file).rsplit("_", 1)[0]

    bases_1 = [base_name(f) for f in files_1]
    bases_2 = [base_name(f) for f in files_2]

    if bases_1 != bases_2:
        raise ValueError("Mismatch between _1 and _2 FASTQ file pairs")

    # Paths for the combined files
    sample_out_folder = os.path.join(sample_out_folder, "combined")
    os.makedirs(sample_out_folder, exist_ok=True)
    combined_1 = os.path.join(sample_out_folder, "combined_1.fastq.gz")
    combined_2 = os.path.join(sample_out_folder, "combined_2.fastq.gz")

    fastq_files.extend([combined_1, combined_2])

    # Concatenate _1 files
    if not os.path.exists(combined_1):
        with open(combined_1, "wb") as w1:
            for f in files_1:
                with open(f, "rb") as r:
                    shutil.copyfileobj(r, w1)

    # Concatenate _2 files
    if not os.path.exists(combined_2):
        with open(combined_2, "wb") as w2:
            for f in files_2:
                with open(f, "rb") as r:
                    shutil.copyfileobj(r, w2)
