...
from varseek.utils import is_program_installed, vcf_to_dataframe, assign_transcript_and_cds
from RLSRWP_2025.utils import make_transcript_df_from_gtf
...
if not os.path.isdir(vk_ref_out) or len(os.listdir(vk_ref_out)) == 0:
    if not os.path.exists(variants):  # transcriptome variants don't exist
        if not os.path.exists(geuvadis_true_vcf):  # true VCF doesn't exist
            if not os.path.exists(geuvadis_preliminary_vcf):  # plink pseudo-VCF doesn't exist
                if not is_program_installed(plink):
                    raise ValueError("plink is required to run this script. Please install plink and ensure that it is in your PATH, or provide the path in this script.")
                
                # first download Geuvadis plink reference files (bim, bed, fam)
                if not os.path.exists(f"{geuvadis_reference_variants_prefix}.bim") or not os.path.exists(f"{geuvadis_reference_variants_prefix}.bed") or not os.path.exists(f"{geuvadis_reference_variants_prefix}.fam"):
                    if not os.path.exists(f"{geuvadis_reference_variants_prefix}.tar.gz"):
                    ...
                    subprocess.run(["tar", "-xzvf", f"{geuvadis_reference_variants_prefix}.tar.gz", "-C", geuvadis_reference_files_dir], check=True)
                
                # make the pseudo-vcf
                plink_to_preliminary_vcf_command = [plink, "--bfile", "/data/geuvadis_data_base/genome_variants_ground_truth/1kg_phase1_all", "--threads", str(number_of_threads_total), "--recode", "vcf", "bgz", "--out", geuvadis_reference_variants_prefix]
                subprocess.run(plink_to_preliminary_vcf_command, check=True)
            
            # convert preliminary VCF to true VCF
            variants_plink_df = vcf_to_dataframe(geuvadis_preliminary_vcf, additional_columns=True, explode_alt=True, filter_empty_alt=True, verbose=True)

            #!!!!! modify from preliminary to True vcf and save to geuvadis_preliminary_vcf
            variants_vcf_df = ""
        
        #!!! convert genome --> transcriptome
        # convert true VCF to transcriptome variants
        variants_vcf_df.rename(columns={"CHROM": "chromosome", "POS": "start_variant_position_genome"}, inplace=True)
        variants_vcf_df["end_variant_position_genome"] = variants_vcf_df["start_variant_position_genome"] + variants_vcf_df["ALT"].str.len() - 1
        variants_vcf_df = (
            adata_var_exploded.apply(lambda row: assign_transcript_and_cds(row, transcript_df, gtf_df), axis=1)
        )
    
    vk.ref(variants=variants, sequences=sequences, w=w, k=k, out=vk_ref_out, dlist_reference_source=dlist_reference_source, index_out=vcrs_index, t2g_out=vcrs_t2g)
