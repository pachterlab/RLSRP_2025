...
ccle_data_out_base = os.path.join(data_dir, "ccle_data_base")
...
json_path = os.path.join(ccle_data_out_base, "ccle_metadata.json")
...
def download_ccle_total(
    ...
    # count number of dirs in ccle_data_out_base
    if not all_files_downloaded:
        if failed_downloads:
            print(f"Failed downloads in {sample}: {failed_downloads}")
        else:
            number_of_dirs = len(os.listdir(ccle_data_out_base))
