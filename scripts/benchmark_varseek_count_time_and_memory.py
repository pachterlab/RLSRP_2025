...
for number_of_reads in number_of_reads_list:
    ...
    #     argparse_flags = f"--index {vk_ref_small_k_index_path} --t2g {vk_ref_small_k_t2g_path} --technology bulk --threads {threads} -k {k_small} --out {vk_count_out_tmp} -k {k_small} --kb_count_reference_genome_out_dir {kb_count_reference_genome_out_dir} --disable_clean --fastqs {fastq_output_path}"
    #     print(f"python3 {vk_count_script_path} {argparse_flags}")
    #     if not dry_run:
    #         _ = report_time_and_memory_of_script(vk_count_script_path, output_file = output_file, argparse_flags = argparse_flags, script_title = script_title)
